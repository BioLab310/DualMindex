...
void calc_SArangeSeq(const sFMindex &mem,const char *read,uint32_t *x,uint32_t *y)
{
	char ch;
	...
	ch = read[i];
	p[0] = calc_C(mem,ch) + 1;
//	p[1] = calc_C(mem,C_next(ch)) + 1;//modified by cyy
	p[1] = calc_C(mem,C_next(ch));
	i--;
	while(p[0] <= p[1] && i >= 0)
	{
		ch = read[i];
...
void calc_SArangeSeq_new(const sFMindex &mem,const char *read,uint32_t *x,uint32_t *y,uint32_t l)
{
	char ch;
	int32_t i = l - 1;
	uint32_t p[2];
...
void calc_SArangeChar(const sFMindex &mem,uint32_t *pre, char ch,uint32_t *x,uint32_t *y)
{
	uint32_t p[2];// = (uint32_t *)malloc(sizeof(uint32_t)*2);
